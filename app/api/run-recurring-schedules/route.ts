// app/api/run-recurring-schedules/route.ts
...
export async function GET() {
  ...
  const { data: settingsData } = await supabase.from("settings").select("value").eq("key", "scraperSettings").limit(1)
  ...
  for (const schedule of dueSchedules) {
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("scrapes").upload(fileName, xlsxBuffer, {
      ...
      await supabase.from("recurring_scrapes").update({ skip_times: (schedule.skip_times || 0) + 1 }).eq("id", schedule.id)
    } catch (err) {
